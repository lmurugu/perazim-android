...
echo "PLACEHOLDER_CHECK=$(grep -rnci 'TODO\|FIXME\|stub\|placeholder\|not implemented\|return null' src/main/java/com/example/app/community/ src/main/java/com/example/app/presentation/Phase3* 2>/dev/null | tail -1)" >> $REPO/forensic/monitor_$DATE.log
echo "DEVICE=$(adb -d shell getprop ro.product.model 2>/dev/null) API=$(adb -d shell getprop ro.build.version.sdk 2>/dev/null) APP=$(adb -d shell pm list packages 2>/dev/null | grep 'com.example.app')" >> $REPO/forensic/monitor_$DATE.log
